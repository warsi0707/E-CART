  ...
  return (
    <div className="flex flex-col gap-4 border p-2 rounded-md border-gray-400">
      {carts && carts.map((cart)=>(
        <div key={cart._id} className="flex justify-between items-end border border-slate-300 shadow-md p-1 rounded-md ">
        <div>
          <Link to={""} className="text-purple-primary hover:underline">{cart.title}</Link>
    ...
    <div className="flex justify-between w-full p-2 text-xl border-t border-gray-400">
      ...
      <button onClick={onNext} className="bg-green-primary text-white text-sm p-2 rounded-md cursor-pointer">{title}</button>
    </div>
